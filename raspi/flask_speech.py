        ...
        try:
            # while self.running:
                if self.is_moving == False:
                    ...
                    #print('コマンドを入力してください')
                    #self.command = input('>> ')
                    self.command = self.stt.run_once()
                    print(self.command)
                    response = self.send_to_llm()
                    result = response.get("result", "")
                    result_dict = self.extract_json_from_content(result)
                    self.tts.speak(result_dict["report"])
                    if result_dict:
                        self.is_moving = True
                        if result_dict["action"] == "forward":
                            self.drive.action(self, "forward", result_dict["forward_cm"])
                        elif result_dict["action"] == "right":
                            self.drive.action(self, "right", result_dict["angle_deg"])
                        elif result_dict["action"] == "left":
                            self.drive.action(self, "left", result_dict["angle_deg"])
                        else:
                            self.drive.action(self, "stop", 0)
                    else:
